...
export async function saveProgramEditor(input: unknown, activate: boolean) {
  ...
  try {
    const draft = await prisma.$transaction(tx => persistProgramDraft(tx, profile.id, input, activate), { timeout: 30000 });
    revalidatePath("/programs");
    revalidatePath("/dashboard");
    return { draft };
  } catch (error) {
    return { error: error instanceof ProgramEditorError ? error.message : "Enregistrement impossible. Ton brouillon est conservé ; réessaie." };
  }
}
